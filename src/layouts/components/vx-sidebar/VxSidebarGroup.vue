<template>
  <div
    ...
    @mouseover="mouseover"
    @mouseout="mouseout">
	<div @click="clickGroup" class="group-header w-full" v-if="group.name!='Management'">
		<span class="flex items-center w-full" >
            <img v-if="!sidebarItemsMin" style="background-color:white;" width="30px" class="mr-8" :src="require(`@/assets/images/icons/${group.icon}.svg`)"/>
            <img v-else style="background-color:white; margin-left: -5px;" width="30px" class="mr-8" :src="require(`@/assets/images/icons/${group.icon}.svg`)"/>
    ...
    <ul ref="items" :style="styleItems" class="vs-sidebar-group-items" v-if="group.name!='Management'">
      <li v-for="(groupItem, index) in group.submenu" :key="index">
        ...
        @clickFromChildComponentGroup="handleClickInParentGroup"
        v-else>
    ...
    <div @click="clickGroup" class="group-header w-full high" v-if="group.name=='Management'" style="">
		<span class="flex items-center w-full">
      <img v-if="!sidebarItemsMin" style="background-color:white;" width="30px" class="mr-8" :src="require(`@/assets/images/icons/${group.icon}.svg`)"/>
            <img v-else style="background-color:white; margin-left: -5px;" width="30px" class="mr-8" :src="require(`@/assets/images/icons/${group.icon}.svg`)"/>
            <span v-show="!sidebarItemsMin" class="truncate mr-3 select-none">{{ $t(group.i18n) || group.name }}</span>
		...
		<feather-icon class="high1" icon="ChevronRightIcon" svg-classes="w-4 h-4 " :class="[{'rotate90' : openItems}, 'feather-grp-header-arrow']" v-show="!sidebarItemsMin" />
		<span class="vs-sidebar--tooltip" >{{ $t(group.i18n) || group.name }}</span>
    </div>
    <ul ref="items" :style="styleItems" class="vs-sidebar-group-items" v-if="group.name=='Management'">
      <li v-for="(groupItem, index) in group.submenu" :key="index">
...
export default {
    ...
    methods: {
        handleClickInParentGroup: function(iconNew) {
            this.$emit("clickFromChildComponentGroup", iconNew);
        },
        clickGroup() {
            if (!this.openHover) {
