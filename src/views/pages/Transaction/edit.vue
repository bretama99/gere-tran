        ...
        <vx-card id="internal-card" class="mt-5" title no-shadow card-border>
          ...
          <div class="vx-row mt-4 ml-4">
            <div class="vx-col w-full">
              <vs-button
                v-if="isGranted('updateTransaction')"
...
export default {
  ...
  methods: {
    ...
    fetchTransaction(){
      ...
      this.isLoading=true
      this.$http
        .get("/api/transactions/"+this.transactionId)
        .then(function(response) {
          thisIns.isLoading=false
          thisIns.record.amount = response.data.data.amount;
          thisIns.record.exRate = response.data.data.ex_rate;
          thisIns.record.type = response.data.data.type;
          thisIns.record.bankId = response.data.data.bank_id;
          thisIns.record.branch = response.data.data.branch;
          thisIns.record.receipt = response.data.data.receipt;
          thisIns.record.transactionDate = response.data.data.transaction_date;
        }).catch(error => {
            thisIns.isLoading=false
        });
    }, 
