<template>
  <vx-card :title="$t('zoneList')">
    ...
    <vx-card id="internal-card" title no-shadow card-border>
      ...
      >
        <template slot="thead">
          ...
          <vs-th sort-key="name">{{$t('ዞባ')}}</vs-th>
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="pr-4 w-1/3" :data="data[indextr].name_tg">{{data[indextr].name_tg}}</vs-td>
            <vs-td class="p-0 pr-4 w-1/3" :data="data[indextr].id">
              <span v-if="isGranted('editZone')" @click="updateZone(data[indextr].id)">
    ...
    <vs-pagination class="m-4" :total="total" v-model="currentPage" @change="fetchZones"></vs-pagination>
  </vx-card>
</template>
<script>
export default {
  ...
  methods: {
    ...
    acceptAlert() {
      const thisIns = this;
      thisIns.$http
        .delete("/api/zones/"+thisIns.zoneId)
        .then(function(response) {
            thisIns.$vs.notify({
            color: "success",
            title: "Status",
