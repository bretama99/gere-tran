<template>
  <vx-card :title="$t('woredaList')">
    ...
    <vx-card id="internal-card" title no-shadow card-border>
      ...
      >
        <template slot="thead">
          ...
          <vs-th sort-key="name">{{$t('ወረዳ')}}</vs-th>
          <vs-th>{{$t('actions')}}</vs-th>
        </template>
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            <vs-td class="w-10">{{(indextr+1)+(limit*(currentPage-1))}}</vs-td>
            <vs-td class="pr-4 w-1/3" :data="data[indextr].name">{{data[indextr].name}}</vs-td>
            <vs-td class="pr-4 w-1/3" :data="data[indextr].name">{{data[indextr].name_tg}} </vs-td>
            <vs-td class="p-0 pr-4 w-1/3" :data="data[indextr].id">
              <span v-if="isGranted('editWoreda')" @click="updateWoreda(data[indextr].id)">
                <vs-avatar color="lightGreen" icon-pack="feather" icon="icon-edit" />
              </span>
              
              <span v-if="isGranted('deleteWoreda')"
                @click=" openConfirm(data[indextr].woredaId)">
    ...
    <vs-pagination class="m-4" :total="total" v-model="currentPage" @change="fetchWoredas"></vs-pagination>
  </vx-card>
</template>
<script>
export default {
  ...
  methods: {
    ...
    assignRestaurant(woredaId){
      this.$router.push({ path: "/pages/assign-restaurant/"+ woredaId});
    },
    fetchWoredas(){
      const thisIns = this;
      this.isLoading=true
      this.$http
        ...
        .then(function(response) {
          ...
          thisIns.woredas = response.data.data;
    ...
    acceptAlert() {
      const thisIns = this;
      thisIns.$http
        .delete("/api/woredas/"+thisIns.woredaId)
        .then(function(response) {
            thisIns.$vs.notify({
            color: "success",
            title: "Status",
    ...
    add: function() {
      this.$router.push({ name: "addWoreda" });
    },
