<template>
  <vx-card :title="$t('contactList')">
      ...
      >
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="pr-4 w-1/4" :data="data[indextr].name">{{data[indextr].name}} {{data[indextr].firstName}}</vs-td>
            <vs-td class="pr-4 w-1/4" :data="data[indextr].email">{{data[indextr].email}}</vs-td>
            <vs-td class="pr-4 w-1/4" :data="data[indextr].phoneNumber">{{data[indextr].phoneNumber}}</vs-td>
    ...
    <vs-pagination class="m-4" :total="total" v-model="currentPage" @change="fetchContacts"></vs-pagination>
  </vx-card>
</template>
<script>
export default {
