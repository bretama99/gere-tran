<template>
  <div id="main-div" class="vx-row">
    <div class="vx-col w-full mb-base">
      <vx-card id="card-id" :title="$t('editMacAddress')">
        <vx-card id="internal-card" class="mt-5" title no-shadow card-border>
          ...
          <div class="vx-row">
              <vs-button v-if="isGranted('editMacAddress')" @click="editMacAddress" color="#292929" class="mr-3 mb-2">{{$t('save')}}</vs-button>
          </div>
        </vx-card>
      </vx-card>
    </div>
...
export default {
  ...
  data() {
    return {
      macAddress: "",
      isLoading: false,
      macAddressId: this.$route.params.id
    };
  },
  methods: {
    fetchMacAddress(){
      this.$http.get("/api/mac-address/"+this.macAddressId)
             .then(response => {
    ...
    editMacAddress() {
      this.$validator.validateAll().then(result => {
       if (result) {
         this.isLoading=true
           this.$http
             .put("/api/mac-address/"+this.macAddressId,
             {
