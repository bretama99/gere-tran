...
const pushIn = document.getElementById('push-in');

console.log(pushIn, "pushIn");

if (document.querySelector('.sidenav-toggler')) {
  let sidenavToggler = document.getElementsByClassName('sidenav-toggler')[0];
  let sidenavHidden = document.getElementsByClassName('g-sidenav-hidden')[0];
  let toggleNavbarMinimize = document.getElementById('navbarMinimize');

  pushIn.style.fontSize = "32px";

  if (sidenavHidden) {
    sidenavToggler.onclick = function() {
      if (!sidenavHidden.classList.contains('g-sidenav-hidden')) {
        sidenavHidden.classList.remove('g-sidenav-pinned');
        sidenavHidden.classList.remove('g-sidenav-show');
        pushIn.classList.remove('text-white');
        sidenavHidden.classList.add('g-sidenav-hidden');
        if (toggleNavbarMinimize) {
          toggleNavbarMinimize.click();
          toggleNavbarMinimize.setAttribute("checked", "true");
        }
      } else {
        sidenavHidden.classList.add('g-sidenav-show');
        sidenavHidden.classList.add('g-sidenav-pinned');
        pushIn.classList.add('text-white');
        sidenavHidden.classList.remove('g-sidenav-hidden');
        if (toggleNavbarMinimize) {
          toggleNavbarMinimize.click();
...
//Set choice File of avatar
const mySidenav = document.getElementById("sidenav-main");
const logoDashboard = document.getElementById("logo-dashboard"); //Logo full by name head of Sidebar 
const liIconeAdmin = document.getElementById("li-icone-admin");
const labelIconeAdmin = document.getElementById("label-icone-admin");
const labelAddRestaurant = document.getElementById("label-add-restaurant");
const imgPushIn = document.createElement("img");
const iconeAdmin = document.getElementById("icone-admin");

if (logoDashboard) {
  logoDashboard.style.display = "none";
      
  pushIn.textContent = ""; //Icone logo simple head of Sidebar (Icone logo simple or icone list)
  imgPushIn.style.display = "flex";
  imgPushIn.setAttribute("src", "./../assets/img/logo.png");
  imgPushIn.style.height = "65px";
  pushIn.appendChild(imgPushIn);
  mySidenav.style.backgroundColor = "linear-gradient(195deg, #393939 0%, #393939 100%)";

  if (iconeAdmin) {
    iconeAdmin.setAttribute("height", "25px");
    iconeAdmin.setAttribute("class", "m-auto");
    iconeAdmin.src = "./../img/user-login-light.png";
    labelAddRestaurant.textContent = "";
  }
}

const makeIconeSidebarMove = (e) => {
    e.stopPropagation();

    mySidenav.style.backgroundImage = "url('./../img/background.png')";
    mySidenav.style.backgroundColor = "linear-gradient(195deg, #393939 0%, #393939 100%)";
    mySidenav.style.backgroundSize = "cover";
    mySidenav.style.backgroundRepeat = "no-repeat";
    mySidenav.style.backgroundPosition = "50%";

    pushIn.textContent = "list";

  //if (labelAddRestaurant) {
    labelAddRestaurant.textContent = "Ajouter un restaurant";

    logoDashboard.style.display = "flex";
    logoDashboard.style.visibility = "visible";
      
    iconeAdmin.removeAttribute("height", "25px");
    iconeAdmin.setAttribute("height", "29px");
    labelIconeAdmin.removeAttribute("class", "d-none");
    labelIconeAdmin.setAttribute("class", "d-flex");
    labelIconeAdmin.style.paddingLeft = "0.25rem";
    labelIconeAdmin.style.marginLeft = "0.5rem";

    iconeAdmin.removeAttribute("class", "m-auto");
    iconeAdmin.src = "./../img/logo.png";
  //}
}

const makeIconeSidebarOut = () => {
  logoDashboard.style.visibility = "hidden";
  logoDashboard.style.display = "unset";
  logoDashboard.style.display = "none";

  pushIn.textContent = "";
  let imgPushIn = document.createElement("img");
  imgPushIn.style.display = "flex";
  imgPushIn.setAttribute("src", "./../img/logo.png");
  imgPushIn.style.height = "65px";
  pushIn.appendChild(imgPushIn);


    labelIconeAdmin.removeAttribute("class", "d-flex");
    labelIconeAdmin.setAttribute("class", "d-none");

    iconeAdmin.setAttribute("class", "m-auto");
    iconeAdmin.setAttribute("height", "25px");
    iconeAdmin.setAttribute("class", "m-auto");
    iconeAdmin.setAttribute("src", "/src/./assets/img/logo.png");
    iconeAdmin.src = "./../img/logo.png";
    labelAddRestaurant.textContent = "";
}

//if (mySidenav) {
  mySidenav.addEventListener('mousemove', makeIconeSidebarMove);

  mySidenav.addEventListener('mouseout', makeIconeSidebarOut);

  pushIn.addEventListener("click", (e) => {

      if (e.target.getAttribute("active") == "false") {
        e.target.setAttribute("active", "true");
        mySidenav.removeEventListener('mouseout', makeIconeSidebarMove);
        mySidenav.removeEventListener('mouseout', makeIconeSidebarOut);
      } else {
...
function navbarMinimize(el) {
  var sidenavHidden = document.getElementsByClassName('g-sidenav-hidden')[0];
